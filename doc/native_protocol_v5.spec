...
2.2. flags
  ...
  by the mask that allows selecting them):
    ...
    0x16: Multipart flag. The request indicates that it is capable to accept multiple responses and a response
          indicates that there are more responses to follow. This is used by SELECT statements
          to implement bulk read requests, where the client wants to stream all data as quickly as possible
          rather than query page by page.
