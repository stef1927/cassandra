...
public class RangeTombstoneTest
{
    ...
    public void simpleQueryWithRangeTombstoneTest() throws Exception
    {
        ...
        for (int i : live)
            assertTrue("Row " + i + " should be live", partition.getRow(new BufferClustering(bb(i))).hasLiveData(nowInSec));
        for (int i : dead)
            assertFalse("Row " + i + " shouldn't be live", partition.getRow(new BufferClustering(bb(i))).hasLiveData(nowInSec));
    ...
    public void overlappingRangeTest() throws Exception
    {
        ...
        for (int i = 0; i < 5; i++)
            assertTrue("Row " + i + " should be live", partition.getRow(new BufferClustering(bb(i))).hasLiveData(FBUtilities.nowInSeconds()));
        for (int i = 16; i < 20; i++)
            assertTrue("Row " + i + " should be live", partition.getRow(new BufferClustering(bb(i))).hasLiveData(FBUtilities.nowInSeconds()));
        for (int i = 5; i <= 15; i++)
            assertFalse("Row " + i + " shouldn't be live", partition.getRow(new BufferClustering(bb(i))).hasLiveData(nowInSec));
