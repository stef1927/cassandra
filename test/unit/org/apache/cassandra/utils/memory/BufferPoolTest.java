...
public class BufferPoolTest
{
    ...
    public void testGetPut() throws InterruptedException
    {
        ...
        assertNotNull(chunk);
        assertEquals(BufferPool.GlobalPool.CHUNK_SIZE, BufferPool.sizeInBytes());
    ...
    public void testPageAligned()
    {
        for (int i = BufferPool.DEFAULT_BUFFER_SIZE;
                 i <= BufferPool.GlobalPool.BUFFER_SIZE;
    ...
    {
        BufferPool.MEMORY_USAGE_THRESHOLD = BufferPool.GlobalPool.CHUNK_SIZE;
        requestDoubleMaxMemory();
    }
    ...
    {
        BufferPool.MEMORY_USAGE_THRESHOLD = BufferPool.GlobalPool.CHUNK_SIZE / 2;
        requestDoubleMaxMemory();
    }
    ...
    {
        requestUpToSize(BufferPool.DEFAULT_BUFFER_SIZE, 3 * BufferPool.GlobalPool.BUFFER_SIZE, true);
    }
    ...
    {
        final int size = BufferPool.GlobalPool.BUFFER_SIZE + 1;
    ...
    {
        final int size = BufferPool.DEFAULT_BUFFER_SIZE;
        final int numBuffers = BufferPool.GlobalPool.BUFFER_SIZE / size;
    ...
    {
        final int maxFreeSlots = BufferPool.Chunk.NUM_FREE_SLOTS;

        final int[] idxs = new int[maxFreeSlots];
        for (int i = 0; i < maxFreeSlots; i++)
            idxs[i] = i;

        doTestFrees(4096, maxFreeSlots, idxs, 0);
    }
    ...
    {
        doTestRandomFrees(12345567878L, 0);

        BufferPool.reset();
        doTestRandomFrees(20452249587L, 0);

        BufferPool.reset();
        doTestRandomFrees(82457252948L, 0);

        BufferPool.reset();
        doTestRandomFrees(98759284579L, 0);

        BufferPool.reset();
        doTestRandomFrees(19475257244L, 0);
    }

    private void doTestRandomFrees(long seed, int droppedChunks)
    {
        final int maxFreeSlots = BufferPool.Chunk.NUM_FREE_SLOTS;
        ...
        doTestFrees(4096, maxFreeSlots, idxs, droppedChunks);
    }

    private void doTestFrees(final int size, final int maxFreeSlots, final int[] toReleaseIdxs, final int droppedChunks)
    {
        ...
        int freeSize = BufferPool.GlobalPool.BUFFER_SIZE - maxFreeSlots * size;
        ...
        if (chunk.offset() > 0)
            assertTrue(chunk.compactFreeSlots());

        assertEquals(droppedChunks * size, chunk.offset());
    }

    @Test
    public void testDifferentSizeFrees()
    {
        int[] sizes = new int[] {
            5, 1024, 4096, 8, 16000, 78, 512, 256, 63, 55, 89, 90, 255, 32, 2048, 128
        };

        assertTrue(sizes.length <= BufferPool.Chunk.NUM_FREE_SLOTS);
        ...
        assertTrue(sum <= BufferPool.GlobalPool.BUFFER_SIZE);
    ...
    @Test
    public void testFreeSlotsExhausted()
    {
        final int size = 64;
        int[] sizes = new int[BufferPool.Chunk.NUM_FREE_SLOTS * 2];
        ...
        final int expectedOffset = (BufferPool.Chunk.NUM_FREE_SLOTS - 1) * size;
        assertTrue(chunk.compactFreeSlots());
    ...
    {
        final int size = 4096;
        final int numBuffersInChunk = BufferPool.GlobalPool.BUFFER_SIZE / size;
