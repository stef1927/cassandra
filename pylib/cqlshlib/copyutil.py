...
class ExportProcess(ChildProcess):
    ...
    def init_feeder_thread(self):
        ...
        It is important that the order of the responses on the queue is respected, or else the parent process may
        kill off worker processes before it has received all the pages of the last token range.
        """
        def feed_errors():
            while True:
                if self.has_response.wait(1):
                    self.has_response.clear()
                    while self.responses:
                        self.outmsg.send(self.responses.pop())

        self.responses = deque()
        self.has_response = threading.Event()
