...
public class Validator implements Runnable
{
    ...
    {
        assert Range.isInRanges(row.key.getToken(), desc.ranges) : row.key.getToken() + " is not contained in " + desc.ranges;
        ...
        if (!findCorrectRange(row.key.getToken()))
        {
            // redo from start once in case of wrapping range
            ranges = trees.invalids();
            findCorrectRange(row.key.getToken());
        }

        assert range.contains(row.key.getToken()) : "Token not in MerkleTree: " + row.key.getToken();
