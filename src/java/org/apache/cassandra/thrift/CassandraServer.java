...
public class CassandraServer implements Cassandra.Iface
{
    ...
    private ClusteringIndexFilter toInternalFilter(CFMetaData metadata, ColumnParent parent, SliceRange range)
    {
        if (metadata.isSuper() && parent.isSetSuper_column())
            return new ClusteringIndexNamesFilter(FBUtilities.singleton(new BufferClustering(parent.bufferForSuper_column()), metadata.comparator), range.reversed);
        else
        ...
        try
        {
            if (predicate.column_names != null)
            {
                if (metadata.isSuper())
                {
                    ...
                    {
                        return new ClusteringIndexNamesFilter(FBUtilities.singleton(new BufferClustering(parent.bufferForSuper_column()), metadata.comparator), false);
                    }
                    else
                    {
                        NavigableSet<Clustering> clusterings = new TreeSet<>(metadata.comparator);
                        for (ByteBuffer bb : predicate.column_names)
                            clusterings.add(new BufferClustering(bb));
    ...
    private ColumnFilter makeColumnFilter(CFMetaData metadata, Slices slices)
    {
        ...
        if (metadata.isStaticCompactTable() && !columns.statics.isEmpty())
        {
            ...
            {
                if (slices.selects(new BufferClustering(def.name.bytes)))
                    builder.add(def);
            }
    ...
    throws InvalidRequestException, NotFoundException, UnavailableException, TimedOutException
    {
        ...
        try
        {
            ...
            if (metadata.isSuper())
            {
                ...
                filter = new ClusteringIndexNamesFilter(FBUtilities.singleton(new BufferClustering(column_path.super_column), metadata.comparator),
                                                  false);
            }
            else
            {
                ...
                if (cellname.clustering == Clustering.STATIC_CLUSTERING)
                {
                    ...
                    builder.add(metadata.compactValueColumn());
                    columns = builder.build();
                    filter = new ClusteringIndexNamesFilter(FBUtilities.singleton(new BufferClustering(column_path.column), metadata.comparator), false);
                }
                else
                {
    ...
    throws RequestValidationException, UnavailableException, TimedOutException
    {
        ...
        {
            Row row = BTreeRow.emptyDeletedRow(new BufferClustering(column_path.super_column), Row.Deletion.regular(new DeletionTime(timestamp, nowInSec)));
        ...
        try
        {
            ...
            try
            {
                ...
                Clustering pageFrom = metadata.isSuper()
                                    ? new BufferClustering(start_column)
