...
public class IndexSummaryBuilder implements AutoCloseable
{
    ...
    static final long defaultExpectedKeySize = Long.valueOf(System.getProperty(defaultExpectedKeySizeName, "32"));
     ...
     * @param expectedKeySize - an estimate of the average key size
     ...
     */
    public IndexSummaryBuilder(long expectedKeys, double expectedKeySize, int minIndexInterval, int samplingLevel)
    {
        this.samplingLevel = samplingLevel;
        this.startPoints = Downsampling.getStartPoints(BASE_SAMPLING_LEVEL, samplingLevel);

        if (expectedKeySize <= 0 && expectedKeys > 0)
        {   // we should have a valid estimated key size when opening sstables, downsampling them, flushing memtables
            // or compacting existing sstables. For other cases, mostly offline tools that don't use the summary,
            // we must use a default value since it is currently too much work to change all BigTableWriter creation
            // paths to pass in a key size estimate.
            expectedKeySize = defaultExpectedKeySize;
            logger.debug("No estimated partition key size provided to the index summary builder, using default value -D{}={}",
                         defaultExpectedKeySizeName, defaultExpectedKeySize);
        }

        long expectedEntrySize = getEntrySize((long)Math.ceil(expectedKeySize));
        ...
        if (maxExpectedEntriesSize > Integer.MAX_VALUE)
        {
            ...
            logger.warn("min_index_interval of {} is too low for {} expected keys of avg size {}; using interval of {} instead",
                        minIndexInterval, expectedKeys, (long)expectedKeySize, effectiveMinInterval);
    ...
    /**
     * Given a key, return how long the serialized key will be.
     */
    ...
    /**
     * Given a key size, return how long the entry will be, that is add 8 bytes to
     * accomodate for the size of the position.
     */
    private static long getEntrySize(long keySize)
    {
        return keySize + TypeSizes.sizeof((long)0);
    }
    ...
    public IndexSummaryBuilder maybeAddEntry(DecoratedKey decoratedKey, long indexStart, long indexEnd, long dataEnd) throws IOException
    {
        if (keysWritten == nextSamplePosition)
        {
            ...
            {
                // we try our best in the constructor to ensure min_sampling_level is appropriate for the number
                // of keys and key size but, if we still hit this limit then it means there will be a largr gap
                // at the end
                logger.warn("Memory capacity of index summary exceeded (2GB), index summary may have a large gap at the end, " +
                            "consider increasing min_sampling_level");
            }
        }
